...
# Version pour serveur PHP déjà en cours d'exécution
...
# 2. Vérifier que le serveur PHP est accessible
echo -e "${YELLOW}2. Vérification du serveur PHP...${NC}"
if curl -s http://localhost:$PHP_PORT > /dev/null; then
    echo -e "${GREEN}✓ Serveur PHP accessible sur le port $PHP_PORT${NC}\n"
else
    echo -e "${RED}✗ Serveur PHP non accessible sur le port $PHP_PORT${NC}"
    exit 1
fi

# 3. Démarrer localtunnel
echo -e "${YELLOW}3. Démarrage de localtunnel...${NC}"
lt --port $PHP_PORT --print-requests > /tmp/localtunnel.log 2>&1 &
LT_PID=$!
sleep 5
...
echo "URL de vérification : $TUNNEL_URL/whatsapp/webhook.php?hub.mode=subscribe&hub.verify_token=oracle_whatsapp_verify_token_2025&hub.challenge=test"
echo ""
...
echo "1. URL du webhook pour Meta WhatsApp Business :"
echo "   ${GREEN}$TUNNEL_URL/whatsapp/webhook.php${NC}"
echo ""
echo "2. Token de vérification :"
echo "   ${GREEN}oracle_whatsapp_verify_token_2025${NC}"
echo ""
echo "3. Pour tester l'intégration complète :"
echo "   - Allez sur ${GREEN}http://localhost:5174/whatsapp${NC}"
echo "   - Envoyez un message test"
echo "   - Vérifiez l'historique"
echo ""
echo "4. Pour arrêter le tunnel :"
echo "   kill $LT_PID"
echo ""
echo -e "${YELLOW}Le tunnel reste actif. Appuyez sur Ctrl+C pour arrêter.${NC}"
echo ""
echo -e "${GREEN}Logs en temps réel :${NC}"

# Fonction de nettoyage
cleanup() {
    echo -e "\n${YELLOW}Arrêt du tunnel...${NC}"
    kill $LT_PID 2>/dev/null
