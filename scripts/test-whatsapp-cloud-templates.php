...
// Récupérer les constantes de configuration depuis les variables d'environnement ou un fichier de configuration
// À adapter selon votre système de configuration
$WHATSAPP_API_VERSION = 'v22.0';  // Version de l'API WhatsApp
$WHATSAPP_PHONE_NUMBER_ID = '660953787095211'; // ID du numéro de téléphone
$WHATSAPP_WABA_ID = '664409593123173'; // ID du compte WhatsApp Business
$WHATSAPP_APP_ID = '1193922949108494'; // ID de l'application Meta

// Rechercher le token d'accès dans un fichier sécurisé
...
if (file_exists($token_info_path)) {
    ...
    // Extraire le token d'accès en recherchant la ligne contenant "token d'accès utilisateur système"
    $lines = explode("\n", $token_info);
    $token_found = false;
    
    foreach ($lines as $line) {
        if (strpos($line, 'token d\'accès utilisateur système') !== false && strpos($line, ':') !== false) {
            $WHATSAPP_ACCESS_TOKEN = trim(substr($line, strpos($line, ':') + 1));
            $token_found = true;
            echo "Token d'accès trouvé.\n";
            break;
        }
    }
    
    if (!$token_found) {
        die("Impossible de trouver le token d'accès dans le fichier d'informations Meta\n");
    }
} else {
    die("Fichier d'informations Meta introuvable: $token_info_path\n");
}
